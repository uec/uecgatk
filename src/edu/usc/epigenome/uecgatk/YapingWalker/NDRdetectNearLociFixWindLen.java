...
import edu.usc.epigenome.uecgatk.YapingWalker.NDRdetectNearLociFixWindLen.windowsObject;
...
public class NDRdetectNearLociFixWindLen extends LocusWalker<NDRCallContext,windowsObject> implements
...
//	private boolean winPassFilterflag = true;
	...
	private boolean preCompareFlag = false;
	...
	public NDRdetectNearLociFixWindLen() {
	...
	public windowsObject reduce(NDRCallContext value, windowsObject windows) {
		...
		//	System.err.println(windows.windowsPre.size() + "\t" + windows.windowsMid.size() + "\t" + windows.windowsPost.size());
			...
			windows.windowsMid.offerLast(value);
			//preCompareFlag=true;
			//if(windows.windowsMid.peekFirst().getLoc().getStart()>= 861020 && windows.windowsMid.peekFirst().getLoc().getStart()<=861022)
			//System.err.println(windows.windowsPre.size() + "\t" + windows.windowsMid.size() + "\t" + windows.windowsPost.size());
			//
			return windows;
		}
		...
		if(checkNearRodLocation(windows.windowsMid.peekFirst().getLoc(),up,down-1, true, false,true) && checkNearRodLocation(windows.windowsMid.peekLast().getLoc(),up, down-1, true, false,true)){
			
				getNDRFromWindowsBySigTest(windows);
				
				addNDRtoWriter(false);
...
//		logger.info(String.format("Visited windows                                %d", summary.nWindowsVisited));
	...
	public boolean checkNearRodLocation(GenomeLoc locus, int upstream, int downstream, boolean oneRod, boolean noDirection, boolean exactLocus){
		
		GenomeLoc searchLoc = getToolkit().getGenomeLocParser().createGenomeLoc(locus.getContig(), Math.max(locus.getStart() - Math.max(upstream, downstream)-1,0), Math.max(locus.getStart() + Math.max(upstream, downstream),0));
		...
		boolean rodExist = false;
    	 if(locRodIt.hasNext()){
    		...
    		//System.err.println(rodList.size());
    		...
    		if(rodList.size()<=1){
    			if(oneRod){
    				...
    				if(exactLocus){
    					//System.err.println(locus.getStart() + "\t" + bed.getStart() + "\t" + (bed.getStart()-locus.getStart()) + "\t" + (locus.getStart() - bed.getStart()) + "\t" + bed.getStrand().toString() + "\t" + upstream + "\t" + downstream);
    					//System.err.println(locus.getStart() + "\t" +bed.getStart() + "\t" + (downstream) + "\t" + (upstream) );
               		 	if((bed.getStrand()==Strand.POSITIVE && (locus.getStart() == bed.getStart()-upstream || locus.getStart() == bed.getStart() + downstream)) || ((bed.getStrand()==Strand.NEGATIVE && (locus.getStart() == bed.getStart() - downstream || locus.getStart()== bed.getStart() + upstream))) || (noDirection && (bed.getStart()-locus.getStart() == downstream || locus.getStart() - bed.getStart() == upstream))){
	...
	public windowsReturnObject getGchListFromWindow(LinkedList<NDRCallContext> window){
		...
		while(itContext.hasNext()){
			...
			if(num != null && (num.numC + num.numT != 0)){
				int numC = num.numC;
				int numT = num.numT;
	...
	public cytosineStat validateGch(NDRCallContext ncc){
		...
		if((numC + numT) >= NAC.minCTDepth && (numC + numT + numO) >= NAC.minDepth && (numC + numT) != 0){
			returnObj.numC = numC;
			returnObj.numT = numT;
	...
	public double getBinomialSigTest(int k, int n, double pValue){
		...
		//return binomial.probability(k);
		 double p = Double.NaN;
		try {
			p = 1-binomial.cumulativeProbability(k)+binomial.probability(k);
